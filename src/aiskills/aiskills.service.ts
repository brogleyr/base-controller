...
export class AiSkillsService {
  ...
  private formatTranscript(transcript: any): string {
    ...
    if (!Array.isArray(terms)) {
      throw new Error('No terms or courses found in transcript.');
    }
  ...
  async analyzeTranscriptAi(transcript: any): Promise<string> {
    ...
    // Analytical response
    const initialAnalysis = await this.initialAnalysisPrompt(transcriptFormatted);
    const jsonResponse = await this.jsonSchemaPrompt(initialAnalysis);

    try {
      const formattedResponse = this.formatJsonResponse(jsonResponse);
  ...
  private async initialAnalysisPrompt(prompt: string): Promise<string> {
    const apiKey = this.configService.get<string>('OPENAI_API_KEY');
    const openaiUrl = 'https://api.openai.com/v1/chat/completions';

    const depthPrompt = '(Abilities: 5, Tech and Tools: 3, Skills: 3, and Knowledge: 3)';
   ...
   * The json_response contains a list of "Abilities", "Tech and Tools", "Skills", "Knowledge" along with the 
   * courses where a given skill is derived from. It also includes a value "Level" which suggests how familiar the
   * student is with some skill.
   */
  private async jsonSchemaPrompt(initialAnalysis: string): Promise<any> {
    const apiKey = this.configService.get<string>('OPENAI_API_KEY');
    const openaiUrl = 'https://api.openai.com/v1/chat/completions';
