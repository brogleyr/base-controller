...
async function* transcriptBuffersFromZip(zipPath: string) {
    ...
    console.log(`Extracted ${pdfBuffers.length} PDFs from ${zipPath}`);
    for (const [i, buffer] of pdfBuffers.entries()) {
        console.log(`PDF ${i} size: ${buffer.length}`);
        const transcriptBuffers = await PdfLoaderService.splitPdfByTranscripts(buffer, "Student Information");
        console.log(`  Split into ${transcriptBuffers.length} transcript(s)`);
        for (const [j, transcriptBuffer] of transcriptBuffers.entries()) {
            yield { buffer: transcriptBuffer, pdfIndex: i, transcriptIndex: j };
        }
    }
}
...
function checkParseOutput(transcript, studentId) {
    let failures: { pdf: string, errorTitle: string, message: string }[] = [];
    ...
    function check(desc: string, fn: () => void) {
        try {
            ...
            failures.push({ pdf: studentId.studentNumber, errorTitle: desc, message: `${desc} - ${err.message}` });
        }
    }
    ...
    check("transcriptDate", () =>
        expect(transcript.transcriptDate).toMatch(
            /^\d{2}\/\d{2}\/\d{2,4} \d{2}:\d{2}:\d{2} [AP]M$/,
        ),
    );
    ...
    // === Tests ===
    check("tests defined", () => expect(transcript.tests).toBeDefined());

    if (transcript.tests) {
        for (const test of transcript.tests as TestDto[]) {
            check("testTitle", () => expect(test.testTitle).toBeDefined());
            check("testDate", () => {
                if (test.testDate) expect(test.testDate).toMatch(/^\d{2}\/\d{2}\/\d{4}$/)
            });
            check("testScore", () => {
                if (test.testScore) expect(test.testScore).toMatch(/^([\d.]+|P|F)$/)
            });
        }
    }
